# Kourovka finite-group research program

This repository is the umbrella workspace for three finite-group problems from
the 21st Kourovka Notebook:

| Work order | Problem | Short name | Current state |
|---:|---:|---|---|
| 1 | 19.54 | chief factors under the strictly-2-maximal condition | **not started** |
| 2 | 18.68 | composition factors with complemented maximals | **not started** |
| 3 | 21.87 | generation from subgroups of coprime-index gcd | **claimed complete solution; required internal passes, supplemental re-audit, and two independently supplied reports addressed** |

The repository retains research scaffolding for Problems 19.54 and 18.68.
For Problem 21.87 it contains a claimed complete affirmative proof that has
passed the repository's four required internal review rounds and a
supplemental full re-audit.  The
[revised submission candidate `517f7d3`](https://github.com/RichieSater/kourovka-19-54-18-68-21-87/tree/517f7d30510746c381703413a7a049f2951a4491/problem-21-87)
is publicly available for review.  It addresses an independently supplied
mathematical minor-revision report and a subsequent English/exposition pass.
The result has not undergone verified external specialist review and has not
been accepted or formally published.

## Canonical statements

- **19.54.** What are the chief factors of a finite group in which every
  2-maximal subgroup is not \(m\)-maximal for any \(m\geq 3\)?
- **18.68.** What are the nonabelian composition factors of a finite
  nonsoluble group all of whose maximal subgroups have complements?
- **21.87.** Assume that a finite group \(G\) has a nonempty family of
  subgroups, each generated by at most \(d\) elements, whose indices have
  greatest common divisor \(1\). Is it true that \(d(G)\leq d+1\)?

These transcriptions were checked against the July 2026 edition of the 21st
Kourovka Notebook. Exact provenance is recorded in
[`docs/SOURCE-LEDGER.md`](docs/SOURCE-LEDGER.md).

## Start here

1. Read [`TRACKER.md`](TRACKER.md) for portfolio status and the nine required
   solution/review milestones.
2. Read [`ATTACK-ORDER.md`](ATTACK-ORDER.md) for the sequencing and gates.
3. Read [`docs/PREVIOUS-WORK-AUDIT.md`](docs/PREVIOUS-WORK-AUDIT.md) for the
   reusable material from the earlier group-theory projects.
4. Enter the selected problem directory and read `PROBLEM.md`, `ATTACK.md`,
   `REVIEW-PLAN.md`, and `CLAIM-LEDGER.md` before adding code or mathematical
   claims.
5. Run `make check` after structural or tracker edits.

## Layout

```text
problem-19-54/       strictly 2-maximal subgroups and chief factors
problem-18-68/       complemented maximal subgroups and composition factors
problem-21-87/       coprime-index families and generator bounds
shared/              infrastructure that is genuinely property-independent
docs/                provenance, prior-work audit, and research protocol
scripts/             repository-level validation only
tracker/             machine-readable portfolio state and legacy mappings
templates/           initial-solution, revision, and referee report forms
```

Each problem owns its own `gap/`, `tests/`, `data/`, `notes/`, and
`references/` directories. This separation is deliberate: quotient
inheritance, maximal-subgroup tests, and certificate meanings differ between
the three properties and must not be silently conflated.

## Previous work used in the setup

The setup reviewed these local repositories:

- `../kourovka-10-34`
- `../kourovka-19-57-19-58`
- `../monotone-maximal-chains`
- `../Math/kourovka-10.34` (historical exploratory workspace)

Only architecture and candidate techniques are imported at this stage. No
proof from those projects is automatically a proof here.

## Current boundary

An explicit user request resumed Problem 21.87 outside the default attack
order.  Its proof and review artifacts are isolated in `problem-21-87/` and
tracked in the dashboard.  The committed states of Problems 19.54 and 18.68
remain governed by the attack order; no result for either is claimed here.
